...
export function toReactElement(
  el: VirtualChild,
  isTag: boolean = false
): React.ReactElement<VirtualElementProps> | string {
  if (isString(el) || el === null) return el;

  const attributes = el.attributes;
  const children = map(el.children, toReactElement);
  const props =
    children.length > 0 ? assign({}, attributes, { children }) : attributes;
  const type = el.name;
  const key = el.key !== undefined ? el.key : null;
  return isTag
    ? { isTag: true, type, props, key }
    : ({ type, props, key } as any);
}

export function toVirtualElement(
